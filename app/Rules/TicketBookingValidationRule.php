...
class TicketBookingValidationRule implements ValidationRule
{
    /**
    ...
    public function __construct($date, $sessionId)
    {
        $this->date  = $date;
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if ($ticket) {
            ...
            if ($available_quantity == 0 || $booked_quantity > $available_quantity) {
                $fail("The {$ticket->title} is invalid.");
            }
        } else {
            $fail("The {$data['title']} is invalid.");
        }
    }
}
