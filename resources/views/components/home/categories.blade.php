    ...
    @foreach ($this->categories as $category)
        <a href="{{ route('explore', ['category' => $category->id]) }}">
            <div class="flex flex-col items-center space-y-3 group w-full text-center bg-white/5 p-5 h-full rounded-xl">
                <div
                    class="h-17.5 w-17.5 flex group-hover:bg-white/5 duration-300 justify-center items-center bg-black/10 rounded-xl text-white/50 group-hover:text-primary">
